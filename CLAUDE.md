...
3. **Production** — Record date + SKU + No. of pieces. **This is the coil-consumption point.** On save the produced weight is FIFO-allocated across **baby coils** (oldest `dateOfConversion` first, eligible only when baby-coil thickness is within **±0.3 mm** of the SKU thickness); a batch may split across coils. Stored as `coilAllocations: [{babyCoilId, hrCoilId, pieces, weight}]` (the baby coil **and** its mother) with a `status` of `allocated` / `partial` / `unallocated`. The SKU picker is searchable; the assigned-coil dropdown and the FIFO suggestion rows are displayed in **descending MT-available** order (display only — FIFO still decides which coils & how many pieces).
