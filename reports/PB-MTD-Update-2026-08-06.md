...
- **Invoiced MTD (Previous Month)** = July invoiced **through the same day-of-month** (Jul 1–6)
  = 156.7 T, for a like-for-like pace comparison. August is ahead: 183.7 vs 156.7
  (**+27.0 T, +17%**).
- **Total Orders** = MTD Invoice + Confirmed + Non-confirmed = 183.7 + 2.0 + 0.2 = 185.9 T.
  The order book has emptied out — see the advisory below.
...
- **Dispatch D, Production D/D-1 and Orders D/D-1 read 0 because data is behind, not because the
  plant stopped.** Latest dispatch date loaded is **2026-08-05**; latest order date **2026-08-04**;
...
- **Production** uses the same live master recompute as Physical Inventory, so Produced and FG
  can never disagree. August is at 98.2 T against July's 296.9 T over the same 6 days — but with
  production data loaded only through Aug 4, the August figure is incomplete, not a real gap.
...
| 1 | Dispatch D-1 | `theoretical_weight` sum = 25.8 | bundle-line sum = 25.755 | ✅ PASS |
| 2 | Partition — orders | month intake = 93.0 | Σ daily orders Aug = 93.00 | ✅ PASS |
| 2 | Partition — dispatch | invoiced MTD = 183.7 | Σ daily dispatch Aug ≤ D = 183.67 | ✅ PASS |
...
- **Order book near-empty** — Confirmed fell 56.5 → **2.0 T** and Non-confirmed 76.3 → **0.2 T**
  in one day, while Invoiced MTD rose only 54.5 T. Roughly 76 T of non-confirmed disappeared from
  the open book without a matching invoice, which points at a **status/upload change** (lines
  flipped to `delivered`, or the sales sheet re-stated) rather than real conversion. Worth a check
  before anyone reads this as "we're out of orders".
...
- **Baby coil over-consumption** — unfloored `baby_total − consumed` = 5,167.0 − 4,666.5 = 500.5 T,
  ...
  means some baby coils were consumed beyond their recorded slit weight — unchanged from the
  05-Aug report, so it is an existing data issue, not a new one.
...
| Orders Logged D-1 | 60.0 T | 0 T | — (not loaded) |
| Orders Logged D-2 | 28.0 T | 60.0 T | +32.0 |

Reading the move: a day of catch-up loading. Aug 2–4 production landed (**+71.9 T**), drawing
exactly that much out of baby coils — the two tie out. Invoicing added 54.5 T and drained the
confirmed bucket by the same amount, which is clean conversion. The **−76.1 T of non-confirmed**
is the one number that does not tie to anything and should be checked at source.
