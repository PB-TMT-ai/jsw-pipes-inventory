...
- **No uploader on this tab** — dispatch (invoice) data now arrives via the daily **"Upload Sales Excel"** on the Orders tab (the workbook's **Invoice** sheet), processed by `buildDispatchRecords` in **`src/lib/calc.js`** (ticket #190 — every import rule sits where a test can reach it; App.jsx reads the file, calls in, and renders the banner): case-insensitive `pick()` header matching (`mapDispatchRow`), SKU self-heal, per-line dedup, FIFO coil trace. The dynamic `import('xlsx')` and the file reading stay in App.jsx. The Dispatch tab keeps the **Dispatch Records** table + the **Invoice Reconciliation** CSV.
