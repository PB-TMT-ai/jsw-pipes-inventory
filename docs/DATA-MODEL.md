...
| `jsw:campaigns` | `campaigns` | **Campaign** — one plant's monthly production plan. **Unique on `(plant, month)`**, which is also the upsert arbiter: a Campaign is one *mill's* commitment, so Hyderabad's August and Lepakshi's August are different rows. `plant` is stamped once at creation from the header selector and never re-typed, exactly like a pipeline row's. Holds `status` (`draft`/`active`/`closed`, every transition by hand), the hour-budget overrides (`budget_h`, `days_override`, `day_exceptions`) and the provenance of the last **Initiate** press |
| `jsw:campaignRevisions` | `campaign_revisions` | Revision history for one campaign. **Unique on `(campaign_id, revision_no)`**. Revision **1 is the Baseline** — never modified, never deleted, so the month can still be scored against what was originally promised (`docs/adr/0003-…`). **Revise** writes revision *n+1* with a one-line `reason` |
| `jsw:campaignLines` | `campaign_lines` | The **family** targets of one revision — the commitment. **Unique on `(revision_id, family_key)`**. `family_key` is `familyKey(sku)` (`"SHS 50x50"`), never a raw `sku_code`, so two ERP codes for one physical size collapse onto a single planning row. `target_mt` is typed by the operator; `suggested_mt` is what Initiate proposed, kept for comparison |
| `jsw:campaignGauges` | `campaign_gauges` | The **wall-thickness split** of one family line. **Unique on `(line_id, sku_key)`**. Σ gauge targets must reconcile to the family target — the mismatch shows as a test while drafting and blocks only the Draft → Active transition; the family number is never silently rewritten to match an edited gauge |
...
| **Offered** | **All four** — `COIL_INWARD_PLANT_IDS` (Hyderabad and NPMD in ticket #123, Lepakshi and Tapi in ticket #156). Numbering was readied first (ticket #122): `nextCoilNumber` counts each plant's coils separately (NPMD starts at 01 whatever number Hyderabad is on, and so did Lepakshi and Tapi) and `genHRCoilId` takes the plant's prefix (`NPM-`, `LEP-`, `TAP-`, all matching `HYD-`'s three letters), both defaulting to Hyderabad so no existing call or legacy row changed when the gate widened — twice. `coilInwardPlants()` offers the **intersection** of this list and `manufactures`, so a plant is activated by flipping **both** and de-activated by flipping either: reclassifying one stays the one-line change ADR-0004 promised, but *activating* one is two lines in two files |
...
| **Keys present** | `plantKeysIn(rows)` → the distinct stored plant values in a set of rows (order lines, coils, or one dispatch record's entries). Blank, whitespace, missing and an id off the master all collapse to `''`, exactly as `filterByPlant` matches them — so code that GROUPS by plant and code that FILTERS by it read the field one way. Asserted together in `calc.test.js`: the keys partition the rows, losing none |
| **Row filter** | `filterByPlant(rows, selected)` — pass-through on `ALL_PLANTS`; else `rows.filter(r => storedPlant(r) === selected)`. Used for coils, baby coils, productions, orders. Always pass the **sentinel**, never a nullable id: `''` (Unattributed) is a real scope AND a falsy value, so a `scope ? filter(rows, scope) : rows` guard silently widens the one selection where narrowing matters most |
...
| **Client entry point** | `verifyLoginDetails()` in `src/lib/db.js` → `{loginId, plant, role}` or `null`. A credential with no plant comes back as **`ALL_PLANTS`**, never blank — blank is the *other* plant sentinel, `Unattributed`, so returning it would scope the admin to the rows nobody could attribute. The value is fed straight to `filterByPlant`. Returns `null` for a wrong password and **throws** on a network/RPC error, so the UI keeps telling those two apart |
...
| **Three logins** | `admin` (all plants), `hyderabad`, `npmd`. Lepakshi and Tapi get none **even though both run the pipeline** since ticket #156 — their logins were deliberately deferred, so an admin enters for them and `manufactures` grants those two nobody anything yet. Credentials wait until someone there asks. Passwords are set by a **human** in the Supabase SQL editor; no password is in this repository |
